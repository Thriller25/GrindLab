...
// Initialize default header from existing token on load
...
export async function fetchCalcScenariosByFlowsheetVersion(
  ...
  const resp = await api.get<CalcScenarioListResponse>(
    `/api/calc-scenarios/by-flowsheet-version/${flowsheetVersionId}`,
  );
